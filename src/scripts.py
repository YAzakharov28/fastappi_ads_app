...
async def create_initial_roles(session: AsyncSession):
    ...
    right_read_right_own_user = Right(
    ...
    right_read_right_own_ad = Right(
        read=True, write=True, only_own=True, model="Advertisement"
    )
    # Право на чтение любых объявлений
    right_read_any_ad = Right(
        read=True, write=False, only_own=False, model="Advertisement"
    )
    ...
    session.add_all(
        [
            right_read_right_own_user,
            right_full_user,
            right_read_right_own_ad,
            right_read_any_ad,
            right_full_ad,
        ]
    )
